...
RUN cd / && git clone https://github.com/superboySB/gpudrive
WORKDIR /gpudrive
...
RUN ln -sf /gpudrive/.venv/bin/python /usr/local/bin/python \
    && ln -sf /gpudrive/.venv/bin/python /usr/local/bin/python3

# 设置PYTHONPATH，让Python能够找到编译好的包和虚拟环境的site-packages（为了二次开发版本管理，不要写死）
# ENV PYTHONPATH=/workspace/mygpudrive/build:/gpudrive/.venv/lib/python3.11/site-packages:$PYTHONPATH
# 设置虚拟环境变量，确保Python知道它运行在虚拟环境中
ENV VIRTUAL_ENV=/gpudrive/.venv
ENV PATH=/gpudrive/.venv/bin:$PATH
